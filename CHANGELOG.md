...
## [0.3.4] - 2026-02-08
### 🚀 震撼发布：双模引擎 (Vibe & Spec) 与高保真测试体系

#### 新增
- **双模驱动引擎**：在标题栏中央引入震撼的 VIBE/SPEC 切换轨道。
  - **VIBE 模式**：纯净、感性、零干扰的创意空间，物理隔离工具流，防止意图劫持。
  - **SPEC 模式**：理性、严谨的工程堡垒，支持只读工具（读文件、列目录）的静默授权与自动执行。
- **技能系统 2.0**：支持插件化 AI 技能注入，状态持久化恢复，具备“静默执行”铁律。
- **性能巅峰优化**：冷启动耗时从 1s 缩减至 **112ms**，彻底消除 400ms 的主线程阻塞。
- **工业级 UI**：引入渐进式组件加载、Skeleton 骨架屏以及全屏模式氛围切换。

#### 修复与稳定性
- **物理同步方案**：采用 Window 挂载点根除跨模块状态丢失 Bug。
- **死循环抑制**：引入 5s 物理去重锁，彻底解决流式竞态下的重复读取难题。
- **高保真测试**：确立了基于 IPC 报文拦截的 E2E 验证模型，全链路逻辑 100% 闭环。
 (Changelog)
