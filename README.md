...
- **🎉 v0.2.0 重磅发布** - 2025年12月20日 (交互体感跨越式升级)
...
## ✨ v0.2.0 交互革命 - 像 Claude Code 一样丝滑

### 🌊 Claude-style 流式交互系统
- **渐进式代码生成** - AI 的文件操作不再是冰冷的瞬间切换，而是呈现出**打字机式**的逐行生成效果，让您可以实时观察每一行代码的诞生。
- **实时参数提取** - 核心 Rust 后端现在具备**启发式 JSON 解析能力**，即使在 AI 响应尚未结束时，也能精准提取出正在生成的代码内容并立即推送到界面。
- **流式光标反馈** - 工具预览区域增加了动态**脉冲光标**，为您提供极佳的实时视觉确认和心理反馈。

### 🛡️ 灵动任务监控器 (Agent Monitor)
- **自由拖拽与磁力吸附** - 监控器小窗不再固定死板，您可以将其拖动到屏幕任意位置，松开即**智能吸附**到最近的角落。
- **智能布局自适应** - 面板会根据吸附位置自动调整展开逻辑（底部吸附向上展开，顶部吸附向下展开），永远保持界面整洁且不挡代码。
- **全方位中文化支持** - 任务状态、实时日志、初始化提示等所有细节均已接入国际化系统。
- **交互深度打磨** - 精确区分拖拽与点击，优化了日志区域的最大高度，支持在小窗内顺滑查看超长任务日志。

### 📐 智能渲染逻辑
- **总结始终在底部** - 深度重构了渲染引擎，确保 AI 工具盒（如文件写入记录）永远在上方，而最终的总结陈述（Summary）自然落在消息最底部，符合逻辑直觉。
- **极致纯净 UI** - 移除了聊天面板顶部的文字标题，采用极简的 Logo 标识，为代码沟通腾出更多视觉空间。
