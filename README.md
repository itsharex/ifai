...
### 🌟 v0.3.4 新特性：双模驱动灵魂 (Dual-Engine Soul)
- **VIBE 模式 (创意/感性)**：纯净对话空间，物理隔离工具流，拒绝审批干扰，专注于架构与构思。
- **SPEC 模式 (工程/理性)**：严谨执行堡垒，支持“静默读取”自动执行，消除 90% 的多余审批点击。
- **极致交互**：引入骨架屏预加载，彻底消除 AI 思考时的布局抖动 (CLS)。
