...
### 🌟 v0.3.7 新特性：路径感知审批与沉浸式编辑器集成
- **路径感知审批**：基于文件风险等级的智能决策引擎，高危配置强制核验，低危文档静默放行。
- **沉浸式内联预览**：点击文件直接在主编辑器开启 Diff 视图并自动聚焦变更点，支持原位审批。
- **执行层沙箱增强**：Rust 后端物理拦截对 `.git`、`.env` 等核心资产的非法篡改。
- **Monaco 稳定性飞跃**：彻底根治 TextModel 销毁冲突，提升大规模代码编辑的健壮性。
