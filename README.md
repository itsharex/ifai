...
### 🌟 v0.3.6 新特性：UI 工业级重构、PIVO 2.0 与全链路结构化
- **UI 交互重构**：引入模型胶囊面板与增强型多线程管理，提供更专业的编码交互环境。
- **PIVO 2.0 引擎**：基于风险策略的指令引擎，支持执行器解耦、异步预览与并发批处理。
- **全链路结构化**：工具结果直接消费为 PivoProjectTree 图形节点，实现符号级精准上下文注入。
- **NVIDIA GLM-5 支持**：深度适配 NIM 平台的推理模型与思维链协议。
...
| 版本 | 主题 | 核心突破 |
| :--- | :--- | :--- |
| **v0.3.4** | **双模引擎与技能系统** | **Vibe/Spec 双模驱动、插件化技能系统 (Skills)、静默审批自动化、112ms 零阻塞启动、CLS 消除** |
